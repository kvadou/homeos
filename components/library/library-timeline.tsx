import { timeline, tintClasses } from '@/lib/library-data'
  ...
  return (
    <section className="rounded-3xl border border-border/70 bg-card p-6 shadow-sm sm:p-8">
      ...
      <ol className="relative ml-1">
        <span
          aria-hidden
          className="absolute bottom-2 left-[21px] top-2 w-px bg-border"
        />
        {timeline.map(({ year, title, detail, icon: Icon, tint }) => (
          <li key={year} className="relative flex gap-5 pb-7 last:pb-0">
            <span
