...
export async function sendEmail(
  ...
  const from = process.env.WELCOME_FROM_EMAIL // e.g. "GatheredOS <hello@yourdomain.com>"
  if (!apiKey || !from) {
...
<div style="font-family:Georgia,'Times New Roman',serif;max-width:520px;margin:0 auto;padding:32px 24px;color:#0A2E4D;background:#F8F6F2">
  <p style="font-size:22px;letter-spacing:-0.01em;margin:0 0 8px">Welcome to GatheredOS</p>
  <p style="font-family:system-ui,sans-serif;font-size:15px;line-height:1.6;color:#3a4a58;margin:0 0 20px">
    ${greeting} your home finally has a memory. GatheredOS keeps track of your systems,
  ...
  <p style="font-family:system-ui,sans-serif;font-size:15px;line-height:1.6;color:#3a4a58;margin:0 0 24px">
    The best first step: add one system or upload a document, so GatheredOS starts learning your home.
  </p>
  ...
  <p style="font-family:system-ui,sans-serif;font-size:12px;color:#8a97a3;margin:28px 0 0">
    You're receiving this because you created a GatheredOS account.
  </p>
</div>`
}
...
export async function sendWelcomeEmail(to: string, name: string): Promise<SendResult> {
  return sendEmail(to, 'Welcome to GatheredOS', welcomeHtml(name), { idempotencyKey: `welcome-${to}` })
}
