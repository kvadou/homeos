...
function shell(title: string, intro: string, body: string, cta = 'Open HomeOS'): string {
  ...
  <p style="font-size:12px;color:#7a8792;margin:28px 0 0">Manage these emails in <a href="${appUrl}/settings#notifications" style="color:#526979">HomeOS notification settings</a>.</p>
  </div>`
}
...
export async function sendInviteEmail(input: { to: string; homeName: string; inviterName: string; url: string; inviteId: string }) {
  const subject = `${input.inviterName} invited you to ${input.homeName} on HomeOS`
  const html = shell('You’re invited', `${input.inviterName} invited you to share ${input.homeName} in HomeOS.`, `<p style="font-size:14px;line-height:1.6">Use this private, single-use invitation to join the home.</p><a href="${input.url}" style="color:#0a2e4d;font-weight:600">Accept invitation</a>`, 'View invitation')
