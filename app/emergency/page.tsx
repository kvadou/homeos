...
export const metadata: Metadata = { title: 'Emergency · GatheredOS', description: 'Critical information for your home when every second matters.' }
...
export default async function EmergencyPage() {
  ...
  return <AppShell showSearch={false}><div className="space-y-6">
    ...
    {facts.length === 0 && <section className="rounded-3xl border border-border bg-card p-6 text-center"><House className="mx-auto size-6 text-muted-foreground" /><h2 className="mt-3 font-serif text-xl">Complete your emergency record</h2><p className="mx-auto mt-2 max-w-md text-sm text-muted-foreground">Photograph or tell GatheredOS where your water shutoff, gas valve, electrical panel, sump pump, and fire extinguishers are located.</p><Link href="/ask" className="mt-4 inline-flex rounded-xl bg-primary px-4 py-2 text-sm font-medium text-primary-foreground">Add emergency knowledge with Ask</Link></section>}
  </div></AppShell>
}
